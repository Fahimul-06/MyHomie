...
import AddHomie from "../Components/AddHomie.js";
...
function Home() {
  ...
  useEffect(() => {
    const homieId = currentchat?.members.find((m) => m !== user._id);
    const getHomiedetails = async () => {
      try {
        const response = await axios.get(API_URL + "api/users/" + homieId);
        setHomie(response.data);
      } catch (err) { }
    };
    getHomiedetails();
  ...
  return (
    <div className="home">
      {/* Chat Adding Card */}
      <AddHomie addchattoggler={() => { addchatToggler(); }} addchattoggle={addtoggle} />
            ...
            <>
              <div className="chatroom-header">
                <div className="chatroom-chatinfo">
                  <img className='homie-profilepic' src={homie?.photo ? API_URL + "photo/" + homie?.photo : "assets/noavatar.jpg"} alt='' />
